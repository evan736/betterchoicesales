              ...
              {c.status !== 'active' && (
                <button
                  onClick={async (e) => {
                    ...
                    try {
                      await axios.delete(`${API}/api/dialer/campaigns/${c.id}`, { headers: headers() });
                      setCampaigns(campaigns.filter(x => x.id !== c.id));
                      if (selected?.id === c.id) setSelected(campaigns.find(x => x.id !== c.id) || null);
                    } catch (err: any) { alert(err?.response?.data?.error || 'Delete failed'); }
                  }}
                  className="ml-1 p-1 rounded text-slate-600 hover:text-red-400 hover:bg-red-400/10 opacity-0 group-hover:opacity-100 transition-opacity"
