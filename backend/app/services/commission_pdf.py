...
def generate_commission_pdf(sheet_data: dict) -> bytes:
    ...
    info_data = [
        ...
        ["Role:", sheet_data['agent_role'].replace('_', ' ').title(), "Tier:", f"Tier {sheet_data['tier_level']} ({sheet_data['commission_rate'] * 100:.1f}%)"],
    ...
    sum_data = [
        ["New Business Premium", fmt(summary["new_business_premium"]),
    ...
    sum_table = Table(sum_data, colWidths=[2 * inch, 1.5 * inch, 2 * inch, 1.5 * inch])
    sum_table.setStyle(TableStyle([
        ...
        # Chargebacks in red
        ('TEXTCOLOR', (1, 2), (1, 2), colors.HexColor('#dc2626') if summary['chargebacks'] < 0 else colors.black),
        # Grid
    ...
    for item in sheet_data["line_items"]:
        ...
        if item["is_chargeback"]:
            notes = f"CHARGEBACK (eff: {item.get('original_effective_date', '?')[:10]}, {item.get('term_months', '?')}mo term)"
